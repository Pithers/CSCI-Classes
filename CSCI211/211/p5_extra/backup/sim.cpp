...
void run_simulation(Pqueue &arrival_queue, int num_checkers, int break_time, ostream &os)
{
  ...
  for(int i = 0; i < num_checkers; i++)
  {
    ...
    check_array[i].customer_list = new Pqueue;                  //each checker has a customer queue
  }
  ...
  for(clock = 1; num_customers > 0; clock++)
  {
    ...
    while(!shopping_q.empty() && shopping_q.get_priority_of_first() == clock)
    {
      ...
      length = num_customers;                                               //set length to current number of customers
      shortest_line = 0;
      for(int i = 0; i < num_checkers; i++)                                 //find checker with shortest queue
      {
        current = (check_array[i].current_customer == NULL ? 0:1);          //if current checker has a customer, it counts towards that checker's length 
        if((check_array[i].customer_list->length() + current) < length)
        {
          shortest_line = i;                                                //record which checker has the shortest length
          length = check_array[i].customer_list->length() + current;        //update shortest length
        }
      } 
      check_array[shortest_line].customer_list->enqueue(temp_customer, 0);   //place customer on checker with shortest queue 
      temp_customer->print_finish_shop(os, clock, shortest_line);            //print that customer finished shopping and is waiting for specified checker
    }
    ...
    //----Finding Available Checker----
      for(int i = 0; i < num_checkers; i++)
      {
        if(check_array[i].current_customer == NULL && check_array[i].break_left <= clock && !check_array[i].customer_list->empty())    //if checker is available
        {
          temp_customer = check_array[i].customer_list->dequeue();
  ...
  for(int i = 0; i < num_checkers; i++)
  {
    ...
    delete check_array[i].customer_list;
  }
  os << "time = " << clock << endl;
}
