...
int List::sum()
{
  Node *ptr = m_head;
  int sum = 0;
  while(ptr != NULL)      //iterate through all of the nodes and sum each value
  {
    sum = sum + ptr->m_value;
    ptr = ptr->m_next;
  }
...
void List::insert_at_end(int value)
{
  ...
  if(ptr == NULL)          //if there are no nodes insert at front
  {
    m_head = new Node(value, m_head);
  }
  else                     //otherwise iterate through all nodes and create a node at the back
  {
