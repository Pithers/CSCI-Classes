...
int Vlist::lookup(string title)
{
  ...
  while(pnode != NULL)
  {
    if(pnode->vlist_pvideo->get_title() == title)
    {
      pnode->vlist_pvideo->print();
      return 0;
    }
    pnode = pnode->vlist_pnext;
  }
...
int Vlist::remove(string title)
{
  ...
  if((pnode != NULL) && (pnode->vlist_pvideo->get_title() == title))
  {
    vlist_phead = pnode->vlist_pnext; 
    delete pnode;
    return 0;				//succesfully deleted title
  }
  else
  {
    while(pnode != NULL && pnode->vlist_pnext != NULL)
    {
      if(pnode->vlist_pnext->vlist_pvideo->get_title() == title)
      {
        temp = pnode->vlist_pnext;
        pnode->vlist_pnext = pnode->vlist_pnext->vlist_pnext;
        delete temp;
        return 0;
      }
      pnode = pnode->vlist_pnext;
